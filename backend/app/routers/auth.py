...
class AuthController:
    ...
    async def register(self, user_create: UserCreate) -> UserResponse:
        try:
            user = await self.user_service.register_user(self.db, user_create)
    ...
    async def login(self, login_data: UserLogin) -> Token:
        user = await self.user_service.authenticate_user(
            self.db, login_data.phone_number, login_data.password
        )
        if not user:
...
def get_auth_controller(
    ...
    return AuthController(db, user_service)
